...
namespace Lab4
{
    class Program
    {
        ...
        static void RunTask8()
        {
            ...
            // Первый способ: Простой шифр Цезаря
            Console.Write("Введите ключ (сдвиг) для простого шифра Цезаря: ");
            int simpleKey = int.Parse(Console.ReadLine());
            string simpleEncrypted = SimpleCaesarCipherEncrypt(input, simpleKey);
            string simpleDecrypted = SimpleCaesarCipherDecrypt(simpleEncrypted, simpleKey);
            Console.WriteLine("Результат (простой шифр Цезаря):");
            Console.WriteLine("Зашифрованный текст: " + simpleEncrypted);
            Console.WriteLine("Расшифрованный текст: " + simpleDecrypted);


            // Второй способ: Шифр Цезаря с разными сдвигами
            Console.Write("Введите ключи (сдвиги через пробел) для шифра Цезаря с разными сдвигами: ");
            string[] keys = Console.ReadLine().Split(' ');
            int[] shifts = keys.Select(int.Parse).ToArray();
            string multipleShiftEncrypted = CaesarCipherWithMultipleShiftsEncrypt(input, shifts);
            string multipleShiftDecrypted = CaesarCipherWithMultipleShiftsDecrypt(multipleShiftEncrypted, shifts);
            Console.WriteLine("Результат (шифр Цезаря с разными сдвигами):");
            Console.WriteLine("Зашифрованный текст: " + multipleShiftEncrypted);
            Console.WriteLine("Расшифрованный текст: " + multipleShiftDecrypted);

            // Третий способ: Шифр Цезаря с ключевым словом
            Console.Write("Введите ключевое слово для шифра Цезаря с ключевым словом: ");
            string keyword = Console.ReadLine();
            string keywordEncrypted = CaesarCipherWithKeywordEncrypt(input, keyword);
            string keywordDecrypted = CaesarCipherWithKeywordDecrypt(keywordEncrypted, keyword);
            Console.WriteLine("Результат (шифр Цезаря с ключевым словом):");
            Console.WriteLine("Зашифрованный текст: " + keywordEncrypted);
            Console.WriteLine("Расшифрованный текст: " + keywordDecrypted);
        }


        // 1. Простой шифр Цезаря
        public static string SimpleCaesarCipherEncrypt(string text, int key)
        {
            StringBuilder result = new StringBuilder();
            foreach (char c in text)
            {
                if (char.IsLetter(c))
                {
                    ...
                    result.Append((char)((c - offset + key) % 26 + offset));
                }
                else
                {
                    result.Append(c);
        ...
        public static string SimpleCaesarCipherDecrypt(string text, int key)
        {
            return SimpleCaesarCipherEncrypt(text, 26 - key % 26);
        }

        // 2. Шифр Цезаря с разными сдвигами
        public static string CaesarCipherWithMultipleShiftsEncrypt(string text, int[] shifts)
        {
            StringBuilder result = new StringBuilder();
            int keyIndex = 0;
            foreach (char c in text)
            {
                ...
                {
                    char offset = char.IsUpper(c) ? 'A' : 'a';
                    result.Append((char)((c - offset + shifts[keyIndex % shifts.Length]) % 26 + offset));
                    keyIndex++;
                }
                else
                    result.Append(c);
        ...
        public static string CaesarCipherWithMultipleShiftsDecrypt(string text, int[] shifts)
        {
            StringBuilder result = new StringBuilder();
            int keyIndex = 0;
            foreach (char c in text)
            {
                ...
                {
                    char offset = char.IsUpper(c) ? 'A' : 'a';
                    result.Append((char)((c - offset + 26 - (shifts[keyIndex % shifts.Length] % 26)) % 26 + offset));
                    keyIndex++;
                }
                else
                    result.Append(c);
        ...
        public static string CaesarCipherWithKeywordEncrypt(string text, string keyword)
        {
            StringBuilder result = new StringBuilder();
            string normalizedKey = string.Concat(keyword.Where(char.IsLetter)).ToLower();
        ...
        public static string CaesarCipherWithKeywordDecrypt(string text, string keyword)
        {
            StringBuilder result = new StringBuilder();
            string normalizedKey = string.Concat(keyword.Where(char.IsLetter)).ToLower();
            if (normalizedKey.Length == 0) return text;
            ...
            foreach (char c in text)
            {
                if (char.IsLetter(c))
                {
                    char offset = char.IsUpper(c) ? 'A' : 'a';
                    int shift = normalizedKey[keyIndex % normalizedKey.Length] - 'a';
                    result.Append((char)((c - offset + 26 - (shift % 26)) % 26 + offset));
                    keyIndex++;
                }
                else
                    result.Append(c);
        ...
        static void RunTask9()
        {
            ...
            // Первый способ: обработка строки как массива символов
            string resultByCharArray = RemoveWordsByCharArray(input);
            Console.WriteLine("Результат (через массив символов):");
            Console.WriteLine(resultByCharArray);
        ...
        // Первый способ: обработка строки как массива символов
        public static string RemoveWordsByCharArray(string text)
        {
            StringBuilder result = new StringBuilder();
            char[] separators = { ' ', ',', '.', '!', '?', ';', ':' };
            ...
            foreach (string word in words)
            {
                if (word.Length > 0)
                {
                    ...
                    if (!(new char[] { 'a', 'o', 'e', 'i', 'u' }.Contains(firstChar) && firstChar == lastChar))
                    {
        ...
        // Второй способ: использование String и StringBuilder
        public static string RemoveWordsByStringMethods(string text)
        {
            StringBuilder result = new StringBuilder();
            string[] words = text.Split(new char[] { ' ', ',', '.', '!', '?', ';', ':' }, StringSplitOptions.RemoveEmptyEntries);
            foreach (string word in words)
            {
                if (word.Length > 0)
                {
                    char firstChar = char.ToLower(word[0]);
                    char lastChar = char.ToLower(word[word.Length - 1]);
                    if (!(firstChar == 'a' || firstChar == 'o' || firstChar == 'e' || firstChar == 'i' || firstChar == 'u')
                         || firstChar != lastChar)
                    {
                        result.Append(word).Append(" ");
                    }
        ...
        {
            Console.WriteLine("Введите текст для поиска смайлов:");
            string input = Console.ReadLine();

            // Регулярное выражение для поиска смайлов
            string smileyPattern = @"[:)]|:-[)]|\)+";
            // Поиск всех смайлов, подходящих под шаблон
            MatchCollection matches = Regex.Matches(input, smileyPattern);

            Console.WriteLine("Найденные смайлы:");
            foreach (Match match in matches)
            {
                Console.WriteLine(match.Value);
            }
            // Вывод общего количества найденных смайлов
            Console.WriteLine($"Всего найдено смайлов: {matches.Count}");
        }
    }
}
